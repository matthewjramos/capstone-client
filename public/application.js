...
var spriteY = (canvas.height - spriteHeight) / 2;
...
function keyDown(e) {
  if (e.keyCode === 39) rightKey = true;else if (e.keyCode === 37) leftKey = true;
  if (e.keyCode === 38) upKey = true;else if (e.keyCode === 40) downKey = true;
}

function keyUp(e) {
  if (e.keyCode === 39) rightKey = false;else if (e.keyCode === 37) leftKey = false;

  if (e.keyCode === 38) upKey = false;else if (e.keyCode === 40) downKey = false;
}
...
var createGame = function createGame(event) {
  ...
  api.createGameSubmit(data).then(ui.createGameSuccess).catch(ui.createGameError);
  $(event.target).trigger('reset');
  // game
  // game.newGame()
};

var getGame = function getGame(event) {
  event.preventDefault();
  api.getGameSubmit().then(ui.getGameSuccess).catch(ui.getGameError);
};
var findGame = function findGame(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
...
var updateGame = function updateGame(event) {
  ...
  var gameId = $(event.target).closest('ul').attr('data-id');
  // console.log('closest event target is ', ((event.target).closest('ul')))
  // console.log('gameId is ', gameId)
  api.updateGameSubmit(data, gameId).then(ui.updateGameSuccess).catch(ui.updateGameError);
  // $(event.target).trigger('reset')
};
var deleteGame = function deleteGame(event) {
  event.preventDefault();
  var gameId = $(event.target).closest('ul').attr('data-id');
  api.deleteGameSubmit(gameId).then(ui.deleteGameSuccess).catch(ui.deleteGameError);
};
// Game ends
...
var signUpSubmit = function signUpSubmit(event) {
  event.preventDefault();
  var data = getFormFields(event.target);
  api.signUp(data).then(ui.signUpSuccess).catch(ui.signUpError);
  $(event.target).trigger('reset');
};
var signOutSubmit = function signOutSubmit(event) {
  event.preventDefault();
  api.signOut().then(ui.signOutSuccess).catch(ui.signOutError);
};
var changePasswordSubmit = function changePasswordSubmit(event) {
  var data = getFormFields(event.target);
  // console.log(event.target)
  event.preventDefault();
  api.changePassword(data).then(ui.changePasswordSuccess).catch(ui.changePasswordError);
  $(event.target).trigger('reset');
};
// Auth ends
...
// Game starts
var createGameSubmit = function createGameSubmit(data) {
  return $.ajax({
    method: 'POST',
...
var getGameSubmit = function getGameSubmit() {
  return $.ajax({
    method: 'GET',
    url: 'https://wdi-capstone-api.herokuapp.com/games/',
    headers: {
...
var findGameSubmit = function findGameSubmit(data) {
  return $.ajax({
    method: 'GET',
...
var updateGameSubmit = function updateGameSubmit(data, gameId) {
  // console.log('data is', data)
  // console.log('gameId is ', gameId)
  return $.ajax({
    method: 'PATCH',
...
var deleteGameSubmit = function deleteGameSubmit(gameId) {
  return $.ajax({
    url: 'https://wdi-capstone-api.herokuapp.com/games/' + gameId,
    method: 'DELETE',
    headers: {
...
var createGameError = function createGameError(error) {
...
var getGameSuccess = function getGameSuccess(data) {
  ...
  var showGamesHtml = handlebars({ games: data.games });
  $('#content').html('');
  $('#content').append(showGamesHtml);
  // console.log(showGamesHtml)
  // console.log(data.games)
};
var getGameError = function getGameError(error) {
  $('#display').html('Game retrieval unsuccessful', error);
};
var deleteGameSuccess = function deleteGameSuccess(gameId) {
  $('#display').html('Game deleted successfully!');
  $('#content').html('');
};
var deleteGameError = function deleteGameError(error) {
  $('#display').html('Cannot delete game', error);
};
var findGameSuccess = function findGameSuccess(response) {
  $('#display').html('Game found successfully!');
};
var findGameError = function findGameError(error) {
  $('#display').html('Game find unsuccessful', error);
};
var updateGameSuccess = function updateGameSuccess(gameId) {
  $('#display').html('Game updated successfully!');
  $('#content').html('');
};
var updateGameError = function updateGameError(error) {
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "  <div class=\"col-sm-3 game-box\">\n    <h2 data-id=\""
    + alias2(((helper = (helper = helpers.game_id || (depth0 != null ? depth0.game_id : depth0)) != null ? helper : helpers.helperMissing),(typeof helper === "function" ? helper.call(alias1,{"name":"game_id","hash":{},"data":data,"blockParams":blockParams}) : helper)))
    + "\" class=\"my-game\">Score:"
    + alias2(alias3(((stack1 = blockParams[0][0]) != null ? stack1.score : stack1), depth0))
    + "</h2>\n\n    <ul data-id=\""
    + alias2(alias3(((stack1 = blockParams[0][0]) != null ? stack1._id : stack1), depth0))
    + "\">\n\n      <li>\n        <h3>"
    + ((stack1 = helpers["if"].call(alias1,((stack1 = blockParams[0][0]) != null ? stack1._id : stack1),{"name":"if","hash":{},"fn":container.program(2, data, 0, blockParams),"inverse":container.program(4, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "        </h3>\n      </li>\n      <li>\n        <form class=\"update-game\">\n          <input name=\"game[score]\" type=\"integer\" placeholder=\""
    + alias2(alias3(((stack1 = blockParams[0][0]) != null ? stack1.score : stack1), depth0))
    + "\">\n          <input name=\"game[description]\" type=\"text\" placeholder=\"description\">\n          <br>\n          <button type=\"submit\" class=\"btn btn-primary\">Update Game</button>\n        </form>\n      </li>\n      <li>\n        <button  class=\"delete-game btn btn-danger\">Delete Game</button>\n      </li>\n      </ul>\n\n\n\n    </div>\n\n";
